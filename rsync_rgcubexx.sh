...
if [ -f "./rsync_rgcubexx_config.sh" ]; then
  source "./rsync_rgcubexx_config.sh"
else
  echo "Error: rsync_rgcubexx_config.sh not found."
  exit 1
fi
...
show_help() {
  ...
  echo "  - ${0%.sh}_config.sh (script-specific settings, e.g., rsync_rgcubexx_config.sh)"
...
unrename_target_directories() {
  ...
  log_message "Attempting to unrename target directories on rgcubexx (pre-rsync)..."
...
rename_target_directories() {
  ...
  log_message "Attempting to rename target directories on rgcubexx (post-rsync)..."
...
main() {
  ...
  log_message "rgcubexx sync and modifications complete."
